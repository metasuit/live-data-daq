...
def cfg_read_task(acquisition):
    ...
    acquisition.ai_channels.add_ai_voltage_chan("myDAQ1/ai0",
                                              max_val=10,
                                              min_val=-10)  # Cards1
...
def reading_task_callback(task_idx, event_type, num_samples, callback_data):
    ...
    global buffer_in
    if running:
        path = r'C:\Users\MetaSuit Team\Documents\GitHub\03_LiveDataDAQ'
        ...
        f=np.fft.rfftfreq(data[0][1::].size,d=1/100000)
        P=abs(np.fft.rfft(data[0][1::]))
...
while running:  # make this adapt to number of channels automatically
    ...
    ax2.plot(data[0, -sampling_freq_in * 5:].T,'k')
    ax3.plot(data[0, -sampling_freq_in * 5:].T,'b')
...
plt.pause(1/refresh_rate_plot)  # required for dynamic plot to work (if too low, nulling performance bad) Close task to clear connection once done
task_in.close()
